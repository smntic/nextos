...

{
  config = let
    ...
    cppInclude = operator.ternary config.homeModules.precomp-bits.enable "-include ${config.homeModules.precomp-bits.out}/stdc++.h" "";
  in
    {
      # Python was omit so it does not clash with other installed versions
      home.packages = [
        pkgs.cargo
        pkgs.elixir
        pkgs.ghc
          ...
          local build_functions = {
            cpp = function()
              ...
              local command = string.format(
                '${pkgs.gcc}/bin/g++ -DLOCAL ${cppInclude} -Wall -Wextra "%s" -o "%s"',
                filename,
                output_filename
              )
              term_build(command)
